...
cdef int caesar_vigenere(int ctext,int shift):
    
    # t = (c - k) % m // t = plaintext , c = ciphertext , k = shift(n) , m = alphabet length
    
    cdef int int_deciphered
    int_deciphered = (((ctext - 65) - shift) % 26) + 65
    
    return int_deciphered
...
def vigenere(ctext,key):
    ...
    for c in range(len(ctext)):
        str_deciphered.append(chr(caesar_vigenere(ord(ctext[c]),int_key[c % len(int_key)])))
